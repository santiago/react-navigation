...
class Header extends React.PureComponent<Props, State> {
  ...
  _getHeaderTitleString(scene: NavigationScene): ?string {
    const options = this._getScreenOptions(scene);
  ...
  _getBackButtonTitleString(scene: NavigationScene): ?string {
    ...
    const { headerBackTitle } = this._getScreenOptions(lastScene);
  ...
  _getTruncatedBackButtonTitle(scene: NavigationScene): ?string {
    ...
    return this._getScreenOptions(lastScene).headerTruncatedBackTitle;
  }
  ...
  _renderTitleComponent = (props: SceneProps): ?React.Node => {
    // $FlowFixMe
    const options = this._getScreenOptions(props.scene);
